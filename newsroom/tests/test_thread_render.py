...
def test_first_story_gets_first_class_others_dont():
    # The first story in a section is class="first" (CSS suppresses its top rule);
...
def test_eyebrow_links_to_the_thread_page_when_the_url_is_known():
    ...
    assert (
        ...
        'aria-label="Ongoing · day 4 \u2197 how this story developed">'
        '<span class="loc">Ongoing</span> · day 4 \u2197</a></p>'
    ) in out
...
def test_eyebrow_without_a_url_is_plain_text():
    ...
    assert '<p class="eyebrow"><span class="loc">Ongoing</span> · day 4</p>' in out
    assert "\u2197" not in out and 'aria-label="Ongoing' not in out
