"""The retry layer's invariants: one budget shared across attempts and stages, a caller
deadline beats the default budget, a timed-out attempt is retried, and no attempt starts
past the deadline."""
...
class TestTheRunBudgetFitsUnderSystemd:
    def test_the_run_stage_budget_plus_one_attempt_is_under_the_systemd_start_timeout(self):
        # terraform's TimeoutStartSec, pinned here. run_stage stages only: CLUSTER's extract
        # loop and fulltext run outside this bound.
...
class TestNoAttemptStartsPastTheDeadline:
    ...
    def test_a_second_attempt_is_not_started_past_the_deadline(self, tmp_path, monkeypatch):
        # 0.1s must cover asyncio.run's start-up (p99 12ms under load) or attempt 1 never starts.
