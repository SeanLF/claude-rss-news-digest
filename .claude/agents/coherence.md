...
2. For each headline in draft_selections.json (must_know and should_know), check whether it accurately represents the source articles referenced by article_id. article_ids come from the `sources` array (e.g. `sources: [{article_id: "A1"}, ...]`).
