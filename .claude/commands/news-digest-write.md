...
3. **Keep placeholders exactly as shown** (`{{DIGEST_NAME}}`, `{{DATE}}`, `{{TIMESTAMP}}`) - replaced after file is written
...
6. **Cluster emojis** must be: 🌎 Americas, 🌍 Europe, 🌏 Asia-Pacific, 🌍 Middle East & Africa, 🤖 Tech
7. **JSON format for shown_headlines.json** must be an array of objects with headline, tier, and optional cluster keys
