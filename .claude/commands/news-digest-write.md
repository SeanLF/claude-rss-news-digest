...
Read `data/claude_input/timestamp.json` which contains:
- `weekday`: Full weekday name (e.g., "Monday")
- `month`: Full month name (e.g., "January")
- `day`: Day of month without leading zero (e.g., "8")
- `year`: Four-digit year (e.g., "2025")
- `hour_min`: Time in HH:MM format (e.g., "14:30")
- `timezone`: Timezone abbreviation (e.g., "PST", "UTC")
- `iso_datetime`: ISO format for datetime attribute (e.g., "2025-01-08T14:30")
- `filename_timestamp`: UTC timestamp for filename (e.g., "2025-01-08-2230Z")
- `display`: Pre-formatted display string (e.g., "Monday, January 8, 2025 · 14:30 PST")
...
<head>
  ...
  <title>News Digest – [month] [day], [year]</title>
...
<body>
  <header>
    <time datetime="[iso_datetime]">[display]</time>
  ...
  <footer>
    <p>Generated [year]-[month]-[day] [hour_min] [timezone]</p>
...
1. **Read timestamp.json FIRST** - use these values for all date/time fields in the HTML (header time, title, footer)
2. **Use the EXACT CSS provided** - do not modify colors, fonts, or layout
3. **Use the EXACT HTML structure** - sections must be in order: header, summary, Must Know, Should Know, Quick Signals, Below the Fold, footer
