...
-- CAVEAT: `articles_fetched` is NULL for ~1,283 older source_fetches rows (the column
...
h AS (
    SELECT sf.* FROM source_fetches sf, bounds b WHERE sf.run_id >= b.lo
),
streak AS (
    ...
    FROM (
        ...
        FROM h h1
        WHERE NOT h1.is_success
          AND h1.run_id > COALESCE(
                (SELECT MAX(h2.run_id) FROM h h2
                  WHERE h2.source_id = h1.source_id AND h2.is_success), 0)
    ) failing GROUP BY source_id
)
SELECT
    h.source_id,
    COUNT(*)                                                        AS attempts,
    ROUND(100.0 * SUM(h.is_success::int) / COUNT(*), 1)             AS success_pct,
    COALESCE(s.current_fail_streak, 0)                              AS fail_streak,
    SUM((h.articles_fetched IS NOT NULL)::int)                      AS runs_measured,
    SUM(CASE WHEN h.is_success AND h.articles_fetched = 0 THEN 1 ELSE 0 END) AS zero_yield_runs,
    ROUND(100.0 * SUM(CASE WHEN h.is_success AND h.articles_fetched = 0 THEN 1 ELSE 0 END)
        / NULLIF(SUM((h.articles_fetched IS NOT NULL)::int), 0), 1) AS zero_yield_pct,
    ...
    ROUND(AVG(CASE WHEN h.is_success THEN h.articles_fetched END), 1) AS avg_fetched_ok,
    ROUND(AVG(CASE WHEN h.is_success THEN h.articles_kept END), 1)    AS avg_kept_ok,
    ROUND(100.0 * SUM(CASE WHEN h.is_success THEN h.articles_kept END)
        / NULLIF(SUM(CASE WHEN h.is_success THEN h.articles_fetched END), 0), 1) AS keep_pct,
    substr(MAX(COALESCE(h.error, '') COLLATE "C"), 1, 40)           AS last_error
FROM h LEFT JOIN streak s ON s.source_id = h.source_id
GROUP BY h.source_id, s.current_fail_streak
ORDER BY success_pct ASC, zero_yield_pct DESC NULLS LAST, h.source_id COLLATE "C";
