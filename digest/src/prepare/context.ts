...
// The recent-headline context, read for the run's OWN moment (digest_runs.run_at) rather than now():
...
export async function runAt(db: Sql, runId: number): Promise<string> {
  const r = await db.one<{ run_at: string }>("SELECT run_at FROM digest_runs WHERE id=$1", [runId]);
...
export function previousHeadlines(db: Sql, when: string, days = 7): Promise<{ headline: string; tier: string; date: string }[]> {
  return db.all(
    `SELECT COALESCE(original_title, headline) AS headline, tier, ${utcDay("shown_at")} AS date FROM shown_narratives
...
export function yesterdayHeadlines(db: Sql, when: string): Promise<{ headline: string; tier: string }[]> {
  return db.all(
    `SELECT headline, tier FROM shown_narratives
     WHERE run_id = (SELECT id FROM digest_runs WHERE completed_at IS NOT NULL AND run_at < ${at} ORDER BY run_at DESC LIMIT 1)
