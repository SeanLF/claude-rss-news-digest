...
const since = (await db.one<{ t: string }>("SELECT now() AS t"))!.t;
const acts = threadsActivities({ store, dbUrl: dbUrl(), agentsDir: process.env["AGENTS_DIR"] ?? DEFAULT_AGENTS_DIR, config: threadsConfigFrom(process.env), maxAttempts: 1, onUsage: (row) => recordUsage(db, row) });
...
const links = JSON.parse(await store.content(run, THREAD_LINKS)) as { linker_ok: boolean; stories: { label: string; proposed_thread: number | null; outcome: string }[] };
console.log(JSON.stringify({ run, linkerOk: links.linker_ok, stories: links.stories.map((s) => [s.label, s.outcome, s.proposed_thread]), assignments: JSON.parse(await store.content(run, THREAD_ASSIGNMENTS)) as unknown, synthesized: outcomes.length, auditFailures: outcomes.filter((o) => o.auditFailed).length, failures, cost: await runCost(db, run, since) }, null, 2));
