...
import { dbUrl, openDb, type RowOf } from "../store/db.js";
...
for (let run = first; run <= (last ?? first); run++) {
  ...
  const text = (await db.one<Pick<RowOf<"artifacts">, "content">>("SELECT content FROM artifacts WHERE run_id=$1 AND name='coherence_report.json' AND status='current'", [run]))?.content ?? null;
