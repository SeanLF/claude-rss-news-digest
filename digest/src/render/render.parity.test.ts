...
// Both sides through the production targets, so this can only erase differences that render the same
// in every engine the stylesheet supports (the syntax level itself is pinned in render.test.ts).
const canonicalCss = (css: string) => transform({ code: Buffer.from(css), minify: true, filename: "digest.css", targets: CSS_TARGETS }).code.toString().replaceAll(/,\s+/g, ",");
