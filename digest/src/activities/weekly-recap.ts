...
export function weeklyRecapActivity(deps: WeeklyRecapDeps): (runId: number, force?: boolean) => Promise<Pointer | null> {
  return async (runId, force = false) => {
    ...
    try {
      ...
      deps.onUsage?.({ model: spec.model, thinking: spec.thinking, tokens: r.usage, stage: "weekly_recap", runId, costUsd: r.costUsd, durationMs: r.durationMs, numTurns: r.numTurns });
      summary = r.text.trim();
