...
function setup(prior: string | null) {
  const path = freshDb([300, 301]);
  const db = new DatabaseSync(path);
  db.prepare("UPDATE digest_runs SET run_at='2026-09-25 10:25:00' WHERE id=301").run();
  db.exec("CREATE TABLE shown_narratives (id INTEGER PRIMARY KEY, headline TEXT NOT NULL, tier TEXT, run_id INTEGER, original_title TEXT, shown_at DATETIME)");
  const ins = db.prepare("INSERT INTO shown_narratives (headline, tier, original_title, shown_at) VALUES (?, 'must_know', ?, ?)");
  ins.run("Editorial headline", "Ceasefire talks resume", "2026-09-24 10:40:00");
  ins.run("Another", null, "2026-09-20 10:40:00");
  ins.run("Too old", null, "2026-09-10 10:40:00");
  ins.run("Written after the run", null, "2026-09-25 11:00:00");
  db.close();
  const store = new ArtifactStore(path);
  if (prior !== null) store.put(300, WEEKLY_RECAP, prior);
  return { store, path };
}
const week = (d: string, text = `Themes of ${d}.`) => `## Week of ${d}\n${text}\n\n`;
const deps = (s: ReturnType<typeof setup>, query: SdkQuery, maxAttempts = 3) => ({ store: s.store, dbPath: s.path, agentsDir: AGENTS, query, maxAttempts });
afterEach(() => {
  vi.restoreAllMocks();
});
...
describe("weekly recap activity", () => {
  it("a week-old recap gains this week's entry from the titles shown before the run, and the run stores it", async () => {
    const s = setup(week("2026-09-11") + week("2026-09-18"));
    ...
    expect(s.store.get(p!)).toBe(week("2026-09-11") + week("2026-09-18") + week("2026-09-25", "A week of talks."));
  ...
  it("a recap under a week old is carried forward unchanged, with no model call", async () => {
    const s = setup(week("2026-09-19"));
    const calls: Calls = { n: 0 };
    const p = await weeklyRecapActivity(deps(s, fakeQuery("x", calls)))(301);
    expect(s.store.get(p!)).toBe(week("2026-09-19"));
  ...
  it("keeps the last six weeks", async () => {
    ...
    const kept = [...s.store.get(p!).matchAll(/^## Week of (\S+)/gm)].map((m) => m[1]);
  ...
  it("with nothing shown in the week and no earlier recap, stores nothing", async () => {
    ...
    db.exec("DELETE FROM shown_narratives");
    db.close();
