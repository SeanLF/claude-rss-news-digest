...
function setup(structured: unknown) {
  ...
  store.put(300, "articles_1.csv", CSV);
...
describe("cluster activities", () => {
  it("plans batches of 40 in article order and builds the TSV prompt", () => {
  ...
  it("extracts a batch through the runner with the items schema, scopes items to the batch, and persists usable tags", async () => {
    const { store, seen, acts } = setup({ items: [sweden("A1"), sweden("A2"), sweden("A99"), { article_id: "A3", entities: [], keywords: [], primary_event: "" }] });
    ...
    const items = (JSON.parse(store.get(p)) as { items: { article_id: string }[] }).items.map((i) => i.article_id);
  ...
  it("a batch with zero usable items is a retryable failure and stores nothing", async () => {
    const { store, acts } = setup({ items: [{ article_id: "A1", entities: null, keywords: [], primary_event: null }] });
    await expect(acts.extractBatch(300, { index: 0, ids: ["A1", "A2", "A3"] })).rejects.toThrow(/0\/3 articles extracted/);
    expect(store.find(300, "cluster_tags_b0.json")).toBeUndefined();
  });
  it("joins the batches into clusters, title-falls back the tagless minority, and records health", async () => {
    ...
    const { clusters } = JSON.parse(store.get(c)) as { clusters: { story: string; article_ids: string[] }[] };
    expect(clusters.map((x) => x.article_ids)).toEqual([["A1", "A2", "A4", "A5"], ["A3"]]);
    expect(clusters[1]!.story).toBe("Kane on Ballon d'Or list");
    expect(JSON.parse(store.get(store.find(300, "cluster_health.json")!))).toEqual({ articles: 5, title_only_fallback: 1, batches_lost: 0, tags_archived: true });
