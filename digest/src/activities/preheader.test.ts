...
describe("cleanPreheader", () => {
  it("strips a known label, a list marker and wrapping quotes, keeps the first usable line", async () => {
  ...
  it("never decapitates a lead clause that looks like a label", async () => {
    expect(cleanPreheader("WHO: companies filed 235 lawsuits")).toBe("WHO: companies filed 235 lawsuits");
  });
  it("truncates on a word boundary with an ellipsis, within the cap", async () => {
...
describe("preheader activity", () => {
  it("sends the headlines by tier, cleans the reply, and stores it", async () => {
    ...
    const d0 = await store.put(300, "draft_s00.json", JSON.stringify({ plan: { index: 0, tier: "must_know", storyIds: ["A1"], contextIds: ["A1"] }, story: { headline: "Russia votes", summary: "s", sources: [] } }));
    const d1 = await store.put(300, "draft_s01.json", JSON.stringify({ plan: { index: 1, tier: "should_know", storyIds: ["A2"], contextIds: ["A2"] }, story: { headline: "Yen jumps", summary: "s", sources: [] } }));
    ...
    expect(preheaderLine(await store.get(p))).toBe("Russia votes as the yen jumps");
