...
export async function feedHealthAlert(db: Sql, runId: number, fetchSet: readonly string[], threshold: number): Promise<Extract<AlertRequest, { kind: "source-health" }> | null> {
  const recent = await db.all<{ id: string }>("SELECT DISTINCT source_id AS id FROM source_health WHERE recorded_at > now() - interval '7 days'");
  const fetched = new Set(fetchSet);
  const failing: [string, number][] = [];
  for (const { id } of recent) {
    if (!fetched.has(id)) continue;
    const rows = await db.all<{ success: boolean }>("SELECT success FROM source_health WHERE source_id = $1 ORDER BY recorded_at DESC, id DESC LIMIT 10", [id]);
  ...
  const r = await db.one<{ n: number }>("SELECT COUNT(*) AS n FROM source_health WHERE run_id = $1 AND NOT success", [runId]);
