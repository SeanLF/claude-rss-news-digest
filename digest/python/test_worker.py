...
def test_a_pass_that_cannot_get_the_lock_in_time_spends_nothing_and_says_busy(monkeypatch):
    # The lock wait sits inside the activity's own start-to-close. Without a bound, a pass queued
    # behind a long one could time out and be stored as a settled "failed" having decoded nothing.
    monkeypatch.setattr(worker, "LOCK_WAIT_S", 0.2)
    calls = []
    monkeypatch.setattr(worker.gnews, "resolve", lambda url, **kw: calls.append(url))
    assert worker._PASS_LOCK.acquire()
    try:
        out = ActivityEnvironment().run(worker.decode_links, ["https://news.google.com/a"])
    finally:
        worker._PASS_LOCK.release()
    assert out == {"links": 1, "decoded": {}, "attempted": 0, "outcome": "busy"}
    assert calls == []
