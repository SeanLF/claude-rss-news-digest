...
`completed`, which the TypeScript side stores as settled: a resume never fetched them again.
"""
...
def test_a_fetch_that_outlives_its_limit_is_no_page_even_if_the_stream_ends_cleanly(monkeypatch):
    ...
    with _trickling(close_delimited, b"<p>partial</p>", interval=0.05) as port:
        with pytest.raises(fulltext.FetchRefused, match="wall-clock"):
            fulltext._download(f"http://127.0.0.1:{port}/a", allow=_exempt(port))
...
class TestTheDeadlineOutcome:
    ...
    def test_a_worker_cut_short_by_its_deadline_reports_deadline_not_completed(self, monkeypatch):
        """Through the real child: the TypeScript side retries `deadline` on a resume."""
        monkeypatch.setattr(settings, "FULLTEXT_KILL_GRACE_S", 10)
        with _trickling(BODY_TRICKLE, interval=0.1) as port:
            start = time.monotonic()
